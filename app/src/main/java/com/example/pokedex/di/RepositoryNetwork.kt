...
val repositoryNetwork = module {
    single<MainRepository> { MainRepositoryImpl(pokedexService = get(), pokemonMapper = get(named("PokemonMapperImpl"))) }
    single<PokemonDetailRepository> { PokemonDetailImpl(pokedexService = get(), pokemonDetailMapper = get(named("PokemonDetailMapperImpl"))) }
}