...
class TypeMapperImpl : ApiMapper<TypeDto, PokemonType> {
    ...
    override fun mapperToDomain(dto: TypeDto): PokemonType {
        return PokemonType(
            id = dto.id ?: 0,
            name = dto.name ?: "",
            doubleDamageFrom = dto.damageRelations?.doubleDamageFrom?.map {
                Type(type = (it.name ?: "").capitalized())
            } ?: emptyList(),
            doubleDamageTo = dto.damageRelations?.doubleDamageTo?.map {
                Type(type = (it.name ?: "").capitalized())
            } ?: emptyList(),
            halfDamageFrom = dto.damageRelations?.halfDamageFrom?.map {
                Type(type = (it.name ?: "").capitalized())
            } ?: emptyList(),
            halfDamageTo = dto.damageRelations?.doubleDamageTo?.map {
                Type(type = (it.name ?: "").capitalized())
            } ?: emptyList(),
            noDamageFrom = dto.damageRelations?.noDamageFrom?.map {
                Type(type = (it.name ?: "").capitalized())
            } ?: emptyList(),
            noDamageTo = dto.damageRelations?.noDamageTo?.map {
                Type(type = (it.name ?: "").capitalized())
            } ?: emptyList(),
        )
    }
